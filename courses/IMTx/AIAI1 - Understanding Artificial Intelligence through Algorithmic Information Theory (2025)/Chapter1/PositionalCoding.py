...
# AIAI    https://aiai.telecom-paris.fr/                                       #
# Institut Mines Telecom  2021      J.-L. Dessalles & Pierre-Alexandre Murena  #
################################################################################

"""
Computes binary positional coding """

import sys
import os
from math import log, ceil
OUTPUTFILE = 'PosCodes.csv'
___Correction = 1
...
def PosCoding0(N):
	" Positional coding, as defined in http://teaching.dessalles.fr/FCI/Chapitre1.html#PositionalCoding "
	return Binary(N+2)[1:]
...
def PosCoding(N):
	" Same as PosCoding0, but with heading 1 "
	return '1 ' + PosCoding0(N)

def PosDecoding(SN):
	try:	
		one, SN1 = SN.split(' ')
		one != '1' and int('')	# trick to trigger ValueError if the code does not start with '1'
	except ValueError:	raise Exception('PosDecoding error with %s' % SN)
	return PosDecoding0(SN1)
...
def RoundNumberCoding(N):
	""" Round numbers such as 30000 are coded as "0 <pos code of 3> <poscode of 4>"
	"""
	SN = str(N)
	...
	if SN1 and SN2:	# really a round number
		...
		# where L is the positional code for the leading digits
		# and D is the positional code for the number of zeros
		# Note the use of white space separators, made possible by the positional code
		# (no syntactic ambiguity: after 0, one expects 2 arguments)
		return '0 %s %s' % (PosCoding0(int(SN1)), PosCoding0(len(SN2)))
	else:	# standard positional coding
		return PosCoding(N)
...
def RoundNumberDecoding(SN):
	try:	
		...
		return PosDecoding0(SN1) * 10 ** PosDecoding0(SN2)
	except ValueError:	# not a round number coding
		return PosDecoding(SN)
...
def DifferenceCoding0(N, NRef):
	""" Difference is coded as "(00|01) <round coding of reference> <pos code of difference>"
	"""
	if N > NRef:	return '00 %s %s' % (RoundNumberCoding(NRef)[2:], PosCoding0(N - NRef))
	if N == NRef:	return RoundNumberCoding(N)
	if N < NRef:	return '01 %s %s' % (RoundNumberCoding(NRef)[2:], PosCoding0(NRef - N))
...
def DifferenceDecoding(SN):	
	...
	try:	
		if Chunks[0] ==		'00':	return RoundNumberDecoding('0 ' + ' '.join(Chunks[1:3])) + PosDecoding0(Chunks[3])
		elif Chunks[0] ==	'01':	return RoundNumberDecoding('0 ' + ' '.join(Chunks[1:3])) - PosDecoding0(Chunks[3])
...
# CODINGMETHODS = [PosCoding, RoundNumberCoding, RoundReferenceCoding]
CODINGMETHODS = [PosCoding, RoundReferenceCoding]
...
def Decoding(SN):
	...
	try:	
		...
		if Chunks[0] == '1':	return PosDecoding(' '.join(Chunks[0:2])), ' '.join(Chunks[2:])
