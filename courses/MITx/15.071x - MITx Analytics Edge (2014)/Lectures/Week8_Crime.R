...
ggplot(DayHourCounts, aes(y = Hour, x = Var1)) + geom_tile(aes(fill = Freq)) + scale_fill_gradient(name="Total MV Thefts", low="white", high="red") + theme(axis.title.y = element_blank())
...
# Redo our plot with GunOwnership:
ggplot(murderMap, aes(x = long, y = lat, group = group, fill = GunOwnership)) + geom_polygon(colour = "black") + scale_fill_gradient(low = "black", high = "red", guide = "legend")
