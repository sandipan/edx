...
def epsilon_greedy(state_vector, epsilon):
    ...
    if explore: 
       ...
       action_index, object_index = np.argmax(q_values_action), np.argmax(q_values_object)
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    q_value_cur_state = 1/2*(q_value_cur_state[0][action_index]+q_value_cur_state[1][object_index])

    # TODO Your code here
    y = reward + (GAMMA * (0 if terminal else maxq_next))
    #print(y, q_value_cur_state)
    loss = F.mse_loss(y, q_value_cur_state) #(y - q_value_cur_state)**2 / 2
...
if __name__ == '__main__':
    ...
    axis.set_xticks(np.arange(0, 200, step=10))
    axis.set_yticks(np.arange(0, 0.60, step=0.02))
    plt.grid(which='both')
    plt.show()
