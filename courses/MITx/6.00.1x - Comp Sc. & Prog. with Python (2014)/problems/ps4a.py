...
def getWordScore(word, n):
    ...
    return len(word) * sum([SCRABBLE_LETTER_VALUES[c] for c in word]) + 50 * (len(word) == n)
...
def updateHand(hand, word):
    ...
    """
    # TO DO ... <-- Remove this comment when you code this function
    updatedHand = {key: value for (key, value) in hand.iteritems()}    
    for c in word:
        updatedHand[c] -= 1
    return updatedHand
...
def isValidWord(word, hand, wordList):
    ...
    """
    # TO DO ... <-- Remove this comment when you code this function
    #return (word in wordList) and ((set([c for c in word]) | set(hand.keys())) == set(hand.keys()))
    
    wordDict = getFrequencyDict(word) 
    return (word in wordList) and sum([hand.get(k, 0) >= wordDict[k] for k in wordDict]) == len(wordDict)
#
