...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        ## YOUR CODE HERE ####
        if a == b or self.is_child(a, b) or self.is_parent(a, b): 
            return (-1, 0)
        a_node, b_node = self.names_to_nodes[a], self.names_to_nodes[b] 
        a_ancestors, b_ancestors = [], []
        while a_node:
            a_ancestors.append(a_node.name)
            a_node = a_node.get_parent()
        while b_node:
            b_ancestors.append(b_node.name)
            b_node = b_node.get_parent()
        #print a_ancestors
        #print b_ancestors
        i, j = len(a_ancestors) - 1, len(b_ancestors) - 1
        while a_ancestors[i] == b_ancestors[j]:
            i, j = i - 1, j - 1
        return (min(i, j), max(i, j) - min(i, j))
