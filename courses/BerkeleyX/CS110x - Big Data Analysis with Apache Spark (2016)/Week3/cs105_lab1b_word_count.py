# Databricks notebook source exported at Tue, 30 Aug 2016 13:00:27 UTC
# MAGIC %md
...
def removePunctuation(column):
    ...
    """
    return trim(regexp_replace(lower(column), r'([^\s\w]|_)+', '')).alias('sentence')
...
shakeWordsDF = (shakespeareDF
                .select(explode(split(shakespeareDF.sentence, '\s+')).alias('word'))).where("word != ''")
