# Databricks notebook source exported at Wed, 31 Aug 2016 06:13:52 UTC
# MAGIC %md
...
def tokenize(string):
    ...
    """
    return [x for x in simpleTokenize(string) if not x in stopwords]
...
def tf(tokens):
    ...
    for token in tokensDict:
      tokensDict[token] /= (1.0 * len(tokens))
...
# TODO: Replace <FILL IN> with appropriate code
corpusRDD = sc.parallelize(amazonRecToToken.collect() + googleRecToToken.collect())
...
idfsToCountRow = idfsSmall.map(lambda x: Row(x[1])) #.take(idfsSmall.count()) #
...
trueDupsRDD = (sims
               .join(goldStandard))
trueDupsCount = trueDupsRDD.map(lambda x: x[1][0] == 1).count()
avgSimDups = trueDupsRDD.map(lambda x: x[1][0]).sum() / float(trueDupsRDD.count())

nonDupsRDD = (sims.subtractByKey(trueDupsRDD)).filter(lambda x: x[1] != 1)
...
def fastCosineSimilarity(record):
    ...
    tokens = record[1]
    #s = cossim({token:value for (token, value) in amazonWeightsBroadcast.value[amazonRec].
