# Databricks notebook source exported at Thu, 18 Aug 2016 08:05:49 UTC
# MAGIC %md
...
#base_df.toPandas().to_csv('C:\\courses\\Edx\\Current\\Intro to Spark\\Week3\\nasa_log.csv')
...
logs_sum_df = (not200DF.groupBy('path')
             .count()
             .sort('count', ascending=False))
...
daily_hosts_df = day_group_hosts_df.groupBy(day_group_hosts_df.day).count().cache()
...
days_with_hosts = daily_hosts_df.select('day').collect()
hosts = daily_hosts_df.select('count').collect()
for i in range(daily_hosts_df.count()):
  days_with_hosts[i] = days_with_hosts[i][0]
  hosts[i] = hosts[i][0]
...
avg_daily_req_per_host_df = (total_req_per_day_df.join(
      daily_hosts_df, daily_hosts_df['day'] == total_req_per_day_df['day'], 'outer'
  ).select(
      total_req_per_day_df['day'], 
      (total_req_per_day_df['count']/daily_hosts_df['count']).alias('avg')
  )
).cache()
...
avgs = (avg_daily_req_per_host_df.select('avg').collect())
for i in range(avg_daily_req_per_host_df.count()):
...
for i in range(errors_by_date_sorted_df.count()):
...
for i in range(hour_records_sorted_df.count()):
  hours_with_not_found[i] = hours_with_not_found[i][0]
  not_found_counts_per_hour[i] = not_found_counts_per_hour[i][0]
