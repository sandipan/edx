# Databricks notebook source exported at Wed, 3 Aug 2016 19:19:46 UTC
# MAGIC %md
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    """
    return SparseVector(num_ohe_feats, [(ohe_dict_broadcast.value[fval],1) for fval in raw_feats])
...
def create_one_hot_dict(input_df):
    ...
    Returns:
        dict: A dictionary where the keys are (featureID, value) tuples and map to values that are
            unique integers.
    """
    return (input_df.select(explode(input_df.features)).distinct().rdd
                     .map(lambda r: tuple(r[0]))
                     .zipWithIndex()
                     .collectAsMap())
...
def parse_point(point):
    ...
    return [(featureID, features[featureID]) for featureID in range(len(features))]
...
def parse_raw_df(raw_df):
    ...
    # return raw_df.select(split(raw_df.text, ',')[0].alias('label').cast('double'), parse_point_udf(raw_df.text).alias('feature')).cache()

#transformed = raw_df.select(split(raw_df.text, ',')[0].alias('label'))#.show() #.show()
#transformed.printSchema()
#print raw_df.select(split(raw_df.text, ',')[0].alias('label').cast('double')).show() 
#print raw_df.withColumn('text', parse_point_udf(raw_df.text)).alias('feature').show()
...
# TODO: Replace <FILL IN> with appropriate code
#parsed_train_df = parsed_train_df.select(col("feature").alias("features"))
#parsed_train_df.show()
...
def one_hot_encoding(raw_feats, ohe_dict_broadcast, num_ohe_feats):
    ...
    Returns:
        SparseVector: A SparseVector of length num_ohe_feats with indices equal to the unique
            ...
            with values equal to 1.0.
    """
    return SparseVector(num_ohe_feats, [(ohe_dict_broadcast.value[fval],1) for fval in raw_feats if fval in ohe_dict_broadcast.value])
    #return SparseVector(num_ohe_feats, sorted([(ohe_dict_broadcast.value[fval],1) for fval in sorted(raw_feats)]))
    
parsed_validation_df = parse_raw_df(raw_validation_df)
ohe_dict_missing_udf = ohe_udf_generator(ohe_dict_broadcast)
ohe_validation_df =  (parsed_validation_df.select(parsed_validation_df.label, ohe_dict_missing_udf(parsed_validation_df['feature']).alias('features'))).cache()
...
def add_log_loss(df):
    ...
    """
    l1 = when((df.p == 0.0), log(df.p + epsilon)).otherwise(log(df.p))
    l2 = when((df.p == 1.0), log(1 - df.p + epsilon)).otherwise(log(1 - df.p))
      
    return df.select((-df.label * l1 - (1 - df.label) * l2).alias('log_loss'))

#print example_log_loss_df.take(1)
...
class_one_frac_train = (ohe_train_df.map(lambda x: x.label == 1).sum() / float(ohe_train_df.count()))
...
#ohe_train_df.show()
loss_df = add_log_loss(ohe_train_df)
#print loss_df.map(lambda x: x.log_loss).sum() #.groupBy().avg('log_loss').collect()
log_loss_tr_base = (loss_df.map(lambda x: x.log_loss).sum()) / float(ohe_train_df.count())
...
def evaluate_results(df, model, baseline=None):
    ...
    log_loss = (with_log_loss_df.map(lambda x: x.log_loss).sum()) / float(df.count())
    return log_loss
...
# TODO: Replace <FILL IN> with appropriate code
log_loss_val_base = evaluate_results(ohe_validation_df, None, (ohe_train_df.map(lambda x: x.label == 1).sum() / float(ohe_train_df.count())))
...
def get_sparsity(df):
    ...
    """
    sparsity_df = df.select(feature_sparsity_udf(df.features).alias('sparsity'))
    #sparsity_df.show()
    return (sparsity_df.map(lambda x: x.sparsity).sum()) / float(df.count()) 
    #return df.select(feature_sparsity_udf(df.features).alias('sparsity')).groupBy().avg('sparsity').collect()
...
class_one_frac_test = (hash_test_df.map(lambda x: x.label == 1).sum() / float(hash_test_df.count()))
