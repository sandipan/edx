...
print("Solution for dI/dt = -kI")	# in Python 2.7: use no brackets
...
Dt = 1.
while Dt > 1./2**10:
	I_init = N                              # initial population 
	t_init = 0                              # initial time
	t_end = 25#150 #110 #25 #60                # stopping time
	...
	for i in range (1, n_steps + 1):
		I = I_arr[i-1]
		t = t_arr[i-1]
		dIdt = -k*I          # calculate the derivative 
		#print t, dXdt
	...
	if Dt < 1:
		...
		print(Dt, imax, abs(N*np.exp(-k*imax/Dt) - p_arr[imax]), diff_arr[imax]) #np.amax(diff_arr))
		#print Dt, p_arr[imax], I_arr[2*imax], abs(p_arr[imax]-I_arr[2*imax])
		#imax *= 2
	Dt /= 2.
	p_arr = I_arr

plt.axvline(x=6, color='brown', label = 't = 6')
plt.xlabel('t (time in days)')
plt.ylabel('I(t) (number of infected persons in thousands)')
plt.legend()
plt.grid(True)                          # show grid 
plt.show()

print(min(I_arr))
indices = np.where(abs(I_arr-0)<1e-6)
print(indices[0][0])
